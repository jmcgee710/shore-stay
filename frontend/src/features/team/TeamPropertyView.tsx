...
import type { TeamProperty } from './TeamDashboard';
import type { StormAlert } from '../watcher/types';
import { CONDITION_META, SEVERITY_META } from '../watcher/types';
...
export default function TeamPropertyView() {
  const { id } = useParams<{ id: string }>();
  const { user } = useAuth();
  const api = useApi();
  const [tab, setTab] = useState<Tab>('report');
  const [teamProp, setTeamProp] = useState<TeamProperty | null>(null);
  ...
  useEffect(() => {
    api.get<TeamProperty[]>('/api/team/my-properties').then(list => {
      setTeamProp(list.find(p => p.property.id === id) ?? null);
    }).finally(() => setLoading(false));
  }, [id]);

  function validPhotos(inputs: string[]) { return inputs.map(u => u.trim()).filter(Boolean); }
  ...
  async function submitReport(e: FormEvent) {
    ...
    setSaving(true);
    try {
      await api.post(`/api/team/properties/${id}/reports`, { summary, overallCondition: condition, photoUrls: validPhotos(photoInputs), notes: notes || undefined });
      setSuccess('Report submitted — the homeowner can see your check-in.');
  ...
  async function submitAlert(e: FormEvent) {
    ...
    setSaving(true);
    try {
      const al = await api.post<StormAlert>(`/api/team/properties/${id}/alerts`, { alertType, severity, description, photoUrls: validPhotos(alertPhotos), damageCostEstimate: costEstimate ? Number(costEstimate) : undefined });
      setCreatedAlert(al);
  ...
  async function submitDispatch(e: FormEvent) {
    ...
    setSaving(true);
    try {
      await api.post(`/api/team/alerts/${createdAlert.id}/dispatch`, { contractorName, contractorPhone: contractorPhone || undefined, trade, workDescription: workDesc, estimatedCost: dispatchCost ? Number(dispatchCost) : undefined });
  ...
  const propName = teamProp?.property.name ?? 'Property';
  const propAddress = teamProp?.property.address;
  const canAutoDispatch = teamProp?.handsOffMode || isManager;
    ...
    <div style={{ minHeight: '100vh', background: T.bg, fontFamily: 'Inter Tight, Inter, sans-serif' }}>
      <header style={{ background: `linear-gradient(160deg, #061e33 0%, ${T.oceanDeep} 40%, ${T.ocean} 100%)`, padding: '20px 20px 0' }}>
        <div style={{ maxWidth: 640, margin: '0 auto' }}>
          <Link to="/team" style={{ fontSize: 12.5, color: 'rgba(255,255,255,0.5)', textDecoration: 'none' }}>← Dashboard</Link>
          <h1 style={{ fontFamily: 'Fraunces, Georgia, serif', fontSize: 28, fontWeight: 500, letterSpacing: '-0.02em', color: 'white', margin: '8px 0 4px' }}>{propName}</h1>
          <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.5)', marginBottom: 14 }}>{propAddress}</p>
          <div style={{ display: 'flex', gap: 8, marginBottom: 14, flexWrap: 'wrap' }}>
            {isManager && (
              <span style={{ display: 'inline-flex', borderRadius: 999, background: 'rgba(86,207,225,0.2)', border: '1px solid rgba(86,207,225,0.35)', padding: '4px 12px', fontSize: 11.5, fontWeight: 600, color: T.seafoam }}>
                Full management access
              </span>
            )}
            {teamProp?.handsOffMode && (
              <span style={{ display: 'inline-flex', borderRadius: 999, background: 'rgba(86,207,225,0.2)', border: '1px solid rgba(86,207,225,0.35)', padding: '4px 12px', fontSize: 11.5, fontWeight: 600, color: T.seafoam }}>
                Hands-off mode — you can auto-dispatch
              </span>
            )}
          </div>
        </div>
        ...
        <div style={{ maxWidth: 640, margin: '0 auto', display: 'flex', borderBottom: '1px solid rgba(255,255,255,0.15)' }}>
          {(['report', 'alert'] as Tab[]).map(t => (
            <button key={t} onClick={() => { setTab(t); setSuccess(''); }}
              style={{ flex: 1, padding: '14px 8px', fontSize: 14, fontWeight: 500, color: 'white', opacity: tab === t ? 1 : 0.5, borderBottom: tab === t ? `2px solid ${T.seafoam}` : '2px solid transparent', background: 'none', cursor: 'pointer', border: 'none', outline: 'none' }}>
          ...
          <form onSubmit={submitReport} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ padding: 18, borderRadius: 14, background: T.cream, border: `1px solid ${T.line}`, display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
                ...
                <textarea required rows={3} value={summary} onChange={e => setSummary(e.target.value)} placeholder="Describe the overall state of the property…" style={{ ...inp, resize: 'none', display: 'block' }} />
              </div>
              <div>
              ...
              <div>
                ...
                <textarea rows={2} value={notes} onChange={e => setNotes(e.target.value)} placeholder="Anything the homeowner should know…" style={{ ...inp, resize: 'none', display: 'block' }} />
              </div>
            </div>
              ...
              <form onSubmit={submitAlert} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                <div style={{ padding: 18, borderRadius: 14, background: T.cream, border: `1px solid ${T.line}`, display: 'flex', flexDirection: 'column', gap: 16 }}>
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      {SEVERITIES.map(s => (
                        <button key={s} type="button" onClick={() => setSeverity(s)}
                          style={{ flex: 1, padding: '9px 4px', borderRadius: 10, fontSize: 12, fontWeight: 700, cursor: 'pointer', border: severity === s ? `2px solid ${T.ocean}` : `2px solid ${T.line}`, background: severity === s ? 'rgba(15,76,117,0.06)' : 'transparent', color: T.oceanDeep }}>
                  ...
                  <div>
                    ...
                    <textarea required rows={3} value={description} onChange={e => setDescription(e.target.value)} placeholder="Describe what happened and current state…" style={{ ...inp, resize: 'none', display: 'block' }} />
                  </div>
                  <div>
                    <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase', color: T.muted, marginBottom: 8 }}>Photo URLs (optional)</div>
                  ...
                  <div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      <input type="number" min={0} value={costEstimate} onChange={e => setCostEstimate(e.target.value)} placeholder="0" style={{ ...inp, width: 120 }} />
                    </div>
                  </div>
                </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                <div style={{ padding: 18, borderRadius: 14, background: `color-mix(in oklab, ${T.green} 10%, ${T.cream})`, border: `1px solid color-mix(in oklab, ${T.green} 25%, transparent)` }}>
                  ...
                  <p style={{ fontSize: 13.5, color: T.muted, lineHeight: 1.55 }}>
                    {createdAlert.requiresOwnerApproval && !canAutoDispatch ? 'Waiting for homeowner approval before dispatch.' : 'You can dispatch a contractor now.'}
                  </p>
                </div>
                {(canAutoDispatch || !createdAlert.requiresOwnerApproval) && (
                  ...
                  {showDispatch && (
                    <motion.form onSubmit={submitDispatch} initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                      ...
                      <div>
                        ...
                        <textarea required rows={2} value={workDesc} onChange={e => setWorkDesc(e.target.value)} placeholder="Describe the work to be done…" style={{ ...inp, resize: 'none', display: 'block' }} />
                      </div>
